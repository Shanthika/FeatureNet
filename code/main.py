...
def train(assign_model_path=None):
    ...
    pred,color,_ = MODEL_GEN.get_gen_model(pointclouds_pl, is_training, scope='generator',bradius=pointclouds_radius,
                                                          reuse=None,use_normal=True, use_bn=False,use_ibn=False,
                                                          bn_decay=bn_decay,up_ratio=UP_RATIO)

    #get emd loss
    gen_loss_emd,matchl_out = model_utils.get_emd_loss(pred, pointclouds_gt[:,:,:3], pointclouds_radius)
    gen_loss_emd *= 5
    #get repulsion loss
    if USE_REPULSION_LOSS:
        gen_repulsion_loss =( model_utils.get_repulsion_loss4(pred)) *5
        tf.summary.scalar('loss/gen_repulsion_loss', gen_repulsion_loss)
    else:
        gen_repulsion_loss =0.0

    #get color loss
    gen_col_loss = (model_utils.get_col_loss(matchl_out,color,pointclouds_gt[:,:,3:],pointclouds_radius)) * 500

    #get total loss function
    pre_gen_loss =   gen_loss_emd + gen_repulsion_loss   +  gen_col_loss + tf.losses.get_regularization_loss()
    ...
    tf.summary.scalar('loss/gen_col', gen_col_loss)
    ...
    pointclouds_image_color = tf.placeholder(tf.float32, shape=[None, 500, 1500, 1])
    pointclouds_col_summary = tf.summary.image('pointcloud_color', pointclouds_image_color, max_outputs=1)
    ...
    image_merged = tf.summary.merge([pointclouds_input_summary,pointclouds_pred_summary,pointclouds_col_summary,pointclouds_gt_summary])
    ...
    with tf.Session(config=config) as sess:
        ...
        ops = {'pointclouds_pl': pointclouds_pl,
               ...
               'pointclouds_image_color': pointclouds_image_color,
        ...
        # restore_epoch, checkpoint_path = 0,None
        global LOG_FOUT
        if restore_epoch==0:
...
def train_one_epoch(sess, ops, fetchworker, train_writer):
    ...
    for batch_idx in range(fetchworker.num_batches):
        ...
        feed_dict = {ops['pointclouds_pl']: batch_input_data,
                     ...
                     ops['pointclouds_gt_normal']:batch_data_gt[:,:,3:],
                     ops['pointclouds_radius']: radius}
        summary,step, _, pred_val,color_val,gen_loss_emd,gen_col_loss = sess.run( [ops['pretrain_merged'],ops['step'],ops['pre_gen_train'],
                                                            ops['pred'],ops['color'], ops['gen_loss_emd'],ops['gen_col_loss']], feed_dict=feed_dict)
        train_writer.add_summary(summary, step)
        loss_sum.append(gen_loss_emd)
        loss_emd.append(gen_loss_emd)
        loss_sum.append(gen_col_loss)
        loss_col.append(gen_col_loss)
        if(batch_idx==0):
            batch_input_data[:,:,3:] *= 255
            batch_data_gt[:,:,3:] *= 255
            np.savetxt('input.xyz',batch_input_data[0,:,:])
            np.savetxt('gt.xyz',batch_data_gt[0,:,:]) 
            color_val *= 255
            data_provider.save_pl('./check.xyz', np.hstack((pred_val[0, ...],color_val[0, ...])))
            batch_input_data[:,:,3:] /= 255
            batch_data_gt[:,:,3:] /= 255
            color_val /= 255
        ...
        #     pointclouds_image_color = pc_util.point_cloud_three_views(color_val[0, :, :])
        #     pointclouds_image_color = np.expand_dims(np.expand_dims(pointclouds_image_color, axis=-1), axis=0)
        ...
        #                 ops['pointclouds_image_color']: pointclouds_image_color,
    ...
    loss_emd = np.asarray(loss_emd)
    loss_col = np.asarray(loss_col) 
    log_string('step: %d mean gen_loss: %f emd: %f col: %f\n' % (step, round(loss_sum.mean(),4),round(loss_emd.mean(),4),round(loss_col.mean(),4)))
    print ('read data time: %s mean gen_loss: %f emd: %f col: %f' % (round(fetch_time,4), round(loss_sum.mean(),4),round(loss_emd.mean(),4),round(loss_col.mean(),4)))

def prediction_whole_model(data_folder=None,show=False,use_normal=True):
    ...
    pred,color, _ = MODEL_GEN.get_gen_model(pointclouds_ipt, is_training=False, scope='generator', bradius=1.0,
                                      reuse=None, use_normal=True, use_bn=False, use_ibn=False, bn_decay=0.95, up_ratio=UP_RATIO)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i,item in enumerate(samples):
            ...
            pred_pl,col_val= sess.run([pred,color], feed_dict={pointclouds_ipt: input})
            ...
            if show:
                ...
                plt.show()
            data_provider.save_pl(path, np.hstack((pred_pl[0, ...],col_val[0, ...])))
