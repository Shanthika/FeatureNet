...
def get_col_loss(matchl_out,color, gt, radius): 
    batch_size = color.get_shape()[0].value
    matched_out = tf_sampling.gather_point(gt, matchl_out)
    dist = tf.reshape((color - matched_out) ** 2, shape=(batch_size, -1))
    dist = tf.reduce_mean(dist, axis=1, keep_dims=True)
    dist_norm = dist / radius

    col_loss = tf.reduce_mean(dist_norm)
    return col_loss
