...
def placeholder_inputs(batch_size, num_point,up_ratio = 4):
    ...
    pointclouds_gt = tf.placeholder(tf.float32, shape=(batch_size, num_point*up_ratio, 6))
...
def get_gen_model(point_cloud, is_training, scope, bradius = 1.0, reuse=None, use_rv=False, use_bn = False,use_ibn = False,
                  use_normal=True,bn_decay=None, up_ratio = 4,idx=None):
    ...
    ###concat color feature
        with tf.variable_scope('up_layer_col',reuse=reuse):
            new_points_list_col = []
            for i in range(up_ratio):
                concat_feat_col = tf.concat([up_l4_points, up_l3_points, up_l2_points, l1_points, l0_points], axis=-1)
                concat_feat_col = tf.expand_dims(concat_feat_col, axis=2)
                concat_feat_col = tf_util2.conv2d(concat_feat_col, 256, [1, 1],
                                              padding='VALID', stride=[1, 1],
                                              bn=False, is_training=is_training,
                                              scope='fc_col_layer0_%d'%(i), bn_decay=bn_decay)

                new_points_col = tf_util2.conv2d(concat_feat_col, 128, [1, 1],
                                             padding='VALID', stride=[1, 1],
                                             bn=use_bn, is_training=is_training,
                                             scope='col_conv_%d' % (i),
                                             bn_decay=bn_decay)
                new_points_list_col.append(new_points_col)
            net_col = tf.concat(new_points_list_col,axis=1)

        #get the color
        color = tf_util2.conv2d(net_col, 64, [1, 1],
                              padding='VALID', stride=[1, 1],
                              bn=False, is_training=is_training,
                              scope='fc_col_layer1', bn_decay=bn_decay)

        color = tf_util2.conv2d(color, 3, [1, 1],
                             padding='VALID', stride=[1, 1],
                             bn=False, is_training=is_training,
                             scope='fc_col_layer2', bn_decay=bn_decay,
                             activation_fn=None, weight_decay=0.0)  # B*(2N)*1*3
        color = tf.squeeze(color, [2])  # B*(2N)*3
