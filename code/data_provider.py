...
def load_patch_data(h5_filename='../h5_data/Patches_noHole_and_collected.h5', skip_rate = 1,num_point=2048, use_randominput=True, norm=False):
    ...
    file = '/home/cvgws2/shan/PU-Net-master2/data/my_data5/'
    input_file = glob(file + "/*.txt") 
    input_file = [string.split('/')[-1] for string in input_file]
    input_file.sort()
    batch_size = len(input_file)
    input = np.empty((batch_size,4096,6))
    gt = np.empty((batch_size,4096,6))
    for i in range(batch_size):
        input[i] = np.loadtxt(file+input_file[i])
    for i in range(batch_size):
        gt[i] = np.loadtxt(file+input_file[i])
    ...
    print(input[0,:10,:])
    print(gt[0,:10,:])
    return input, gt, data_radius, input_file
